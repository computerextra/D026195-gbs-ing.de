...
import pilotBear from "./ThemeToggle/pilot-bear.svg";
import "./ThemeToggle/style.css";

export default function ThemeToggle({ onClick }: ThemeToggle) {
  const [clicked, handleClicked] = useState(false);

  useEffect(() => {
    const prefersDark = window.matchMedia("(prefers-color-scheme: dark)");
    if (prefersDark.matches) handleClicked(true);
    else handleClicked(false);
  }, []);

  const handleClick = () => {
    handleClicked((prev) => !prev);
    onClick();
  };
  return (
    <button className="toggle" aria-pressed={clicked} onClick={handleClick}>
